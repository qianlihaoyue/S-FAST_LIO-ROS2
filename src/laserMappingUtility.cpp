...
void LaserMapping::readParameters() {
    ...
    nh.param<std::vector<double>>("mapping/extrinsic_T", extrinT, std::vector<double>(3, 0));  // 雷达相对于IMU的外参T（即雷达在IMU坐标系中的坐标）
    nh.param<std::vector<double>>("mapping/extrinsic_R", extrinR, std::vector<double>(9, 0));  // 雷达相对于IMU的外参R
    ...
    nh.param<std::string>("pcd_save/savemap_dir", savemap_dir, std::string(ROOT_DIR) + "PCD/");
