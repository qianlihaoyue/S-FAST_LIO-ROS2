...
std::vector<std::pair<int, float>> SCManager::detectSim(pcl::PointCloud<SCPointType>& _scan_down, int NUM_CANDIDATES) {
    Eigen::MatrixXd curr_desc = makeScancontext(_scan_down);  // v1
    ...
    if (tree_making_period_conter % TREE_MAKING_PERIOD_ == 0)  // 为了节省计算资源
    {
        ...
        // polarcontext_invkeys_to_search_.assign(polarcontext_invkeys_mat_.begin(),
        //                                        polarcontext_invkeys_mat_.end() - NUM_EXCLUDE_RECENT);  // 去除最近的一些帧，避免误匹配
        polarcontext_invkeys_to_search_.assign(polarcontext_invkeys_mat_.begin(), polarcontext_invkeys_mat_.end());  // copy
    ...
    for (int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES; candidate_iter_idx++) {
        ...
        std::pair<int, float> onepair{loop_id, yaw_diff_rad};
